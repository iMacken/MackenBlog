...
@section('page_header')
    <div class="page-title">
        <i class="voyager-data"></i> @if(isset($dataType->id)) @lang('Edit') BREAD @lang('For') {{  $dataType->name }} @lang('Table') @elseif(isset($table)) @lang('Create') BREAD @lang('For') {{  $table }} @lang('Table') @endif
    </div>
    @php
    ...
    <div class="page-content container-fluid">
        <div class="row">
            <div class="col-md-12">
                ...
                <form action="@if(isset($dataType->id)){{ route('admin.database.bread.update', $dataType->id) }}@else{{ route('admin.database.bread.store') }}@endif"
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title">{{ ucfirst($table) }} BREAD @lang('Info')</h3>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    ...
                                    <input type="text" class="form-control" readonly name="name" placeholder="@lang('Name')"
                                           value="@if(isset($dataType->name)){{ $dataType->name }}@else{{ $table }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('Display Name') (@lang('Singular'))</label>
                            ...
                            <div class="row clearfix">
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('Icon') (@lang('optional')) @lang('Use') <a
                                                href="{{ admin_asset('fonts/admin/icons-reference.html') }}"
                                                target="_blank">Voyager Font Class</a></label>
                                    <input type="text" class="form-control" name="icon"
                                           placeholder="@lang('Icon to use for this Table')"
                                           value="@if(isset($dataType->icon)){{ $dataType->icon }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('Model') @lang('Name')</label>
                                    <span class="glyphicon glyphicon-question-sign"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="@lang('ex. App\Models\User, if left empty will try and use the table name')"></span>
                                    <input type="text" class="form-control" name="model_name" placeholder="@lang('Model') @lang('Name')"
                                           value="@if(isset($dataType->model_name)){{ $dataType->model_name }}@else{{ $model_name }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="email">@lang('Controller') @lang('Name')</label>
                                    <span class="glyphicon glyphicon-question-sign"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="@lang('ex. PageController, if left empty will use the BREAD Controller')"></span>
                                    <input type="text" class="form-control" name="controller" placeholder="@lang('Controller') @lang('Name')"
                                           value="@if(isset($dataType->controller)){{ $dataType->controller }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="generate_permissions">@lang('Generate') @lang('Permissions')</label><br>
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="server_side">@lang('Server-side Pagination')</label><br>
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title">@lang('Edit the rows for the :table table below', ['table' => $table]):</h3>
                        </div>

                        <div class="panel-body">
                            <div class="row fake-table-hd">
                                <div class="col-xs-2">@lang('Field')</div>
                                <div class="col-xs-2">@lang('Visibility')</div>
                                <div class="col-xs-2">@lang('Input Type')</div>
                                <div class="col-xs-2">@lang('Display Name')</div>
                                <div class="col-xs-4">@lang('Optional Details')</div>
                            </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                <div class="row row-dd">
                                    <div class="col-xs-2">
                                        ...
                                        <strong>@lang('Type'):</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>@lang('Key'):</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>@lang('Required'):</strong>
                                        @if($data['null'] == "NO")
                                            <span>Yes</span>
                                    ...
                                    <div class="col-xs-2">
                                        <input type="checkbox"
                                               ...
                                               @elseif(!isset($dataRow->browse))
                                                    {{ 'checked="checked"' }}
                                               @endif>
                                        <label for="field_browse_{{ $data['field'] }}">@lang('Browse')</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">@lang('Read')</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">@lang('Edit')</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">@lang('Add')</label><br/>
                                        <input type="checkbox"
                                               ...
                                               name="field_delete_{{ $data['field'] }}" @if(isset($dataRow->delete) && $dataRow->delete){{ 'checked="checked"' }}@elseif($data['key'] == 'PRI')@elseif($data['type'] == 'timestamp' && $data['field'] == 'updated_at')@elseif(!isset($dataRow->delete)){{ 'checked="checked"' }}@endif>
                                                <label for="field_delete_{{ $data['field'] }}">@lang('Delete')</label><br/>
                                    </div>
                                    <div class="col-xs-2">
