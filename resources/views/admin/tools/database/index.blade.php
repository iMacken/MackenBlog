...
@section('css')
    <script type="text/javascript" src="{{ admin_asset('js/vue21.min.js') }}"></script>
    <link rel="stylesheet" href="{{ admin_asset('css/database.css') }}">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="voyager-data"></i> @lang('Database')
        <a href="{{ route('admin.database.create') }}" class="btn btn-success"><i class="voyager-plus"></i>
            @lang('Create New Table')</a>
    </h1>
@stop
                ...
                <table class="table table-striped database-tables">
                    <thead>
                        <tr>
                            ...
                            <th>BREAD/CRUD @lang('Actions')</th>
                            <th style="text-align:right">@lang('Table') @lang('Actions')</th>
                        </tr>
                    </thead>
            ...
            @foreach($tables as $table)
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if($table->dataTypeId)
                                <a href="{{ route('admin.database.bread.edit', $table->name) }}"
                            ...
                            @else
                                <a href="{{ route('admin.database.bread.create', ['name' => $table->name]) }}"
                                   class="btn-sm btn-default">
                                    <i class="voyager-plus"></i> @lang('Add') BREAD @lang('to this table')
                                </a>
                            @endif
                            </div>
                        </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('admin.database.bread.delete', ['id' => null]) }}" id="delete_builder_form" method="POST">
                        {{ method_field('DELETE') }}
