...
@section('head_keywords', $post->tags->pluck('name')->implode(',') . ',')
@section('head_description', $post->excerpt)
    ...
    <section class="container">
        <div class="row">
            <post class="post-content markdown-body">
                ...
                <br>
                <p>本文链接:
                    <a href="{{ route('post.show',array('id'=>$post->slug ? $post->slug : $post->id)) }}">{{ route('post.show',array('id'=>$post->slug ? $post->slug : $post->id)) }}</a>
                </p>
                <p class="well">
