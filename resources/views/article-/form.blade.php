...
<div class="form-group">
	...
	<input type="text" name="title" id="title" class="form-control" value="{{ isset($post) ? $post->title : old('title') }}" autofocus>
</div>

<div class="form-group">
	<label for="slug" class="control-label">别名</label>
	<input type="text" name="slug" id="slug" class="form-control" value="{{ isset($post) ? $post->slug : old('slug') }}" autofocus>
</div>
...
<div class="form-group">
	...
	<input type="text" name="image" id="image" class="form-control" value="{{ isset($post) ? $post->image : old('image') }}" autofocus>
</div>

<div class="form-group">
	<label for="editor" class="control-label">正文</label>
	<textarea type="text" name="content" id="editor" class="form-control" autofocus>{!! isset($post) ? $post->content : old('content') !!}</textarea>
</div>

<div class="form-group">
    <label for="excerpt" class="control-label">摘要</label>
    <textarea type="text" name="excerpt" id="excerpt" class="form-control" autofocus>{!! isset($post) ? $post->excerpt : old('excerpt') !!}</textarea>
</div>
...
<div class="form-group">
    ...
    <select id="tag_list" name="tag_list[]" class="form-control" multiple>
        @foreach($tags as $tag)
            @if(isset($post) && $post->tags->contains($tag->id))
...
<div class="form-group">
    ...
    <select style="margin-top: 5px" id="if_show_comments" name="if_show_comments" class="form-control">
        @php $if_show_comments = isset($post) ? $post->getConfig('if_show_comments', 'true') : 'true'; @endphp
...
<div class="form-group">
    ...
    <select id="if_allow_comment" name="if_allow_comment" class="form-control">
        @php $if_allow_comment = isset($post) ? $post->getConfig('if_allow_comment', 'true') : 'true'; @endphp
...
<div class="form-group">
    <label>
        @php $is_draft = isset($post) ? $post->getConfig('is_draft', 'false') : 'false'; @endphp
    ...
    <label>
        @php $is_original = isset($post) ? $post->getConfig('is_original', 'false') : 'false'; @endphp
...
<div class="form-group date" style="display:none">
    <div class="row">
        <div class="col-sm-12">
            ...
            <input type="text" name="published_at" class="form-control" id='published_at' value="{{ isset($post) ? $post->published_at : old('published_at') }}" autofocus>
        </div>
    </div>
</div>
...
<script>
    ...
    new SimpleMDE({
        ...
        placeholder: 'Please input the post content.',
